    ...
    public partial class CustomerProfile : UserControl
    {
        ...
        private void btnUploadImg_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog()==DialogResult.OK)
            {
                ...
                else
                {
                    ...
                    {
                        var customerToUpdate = entities.customers.Where(s => s.customerID == Login.id);
                        foreach (customer p in customerToUpdate)
        ...
        private void loadAvatar()
        {
            ...
            using (Entities entities = new Entities())
            {
                try
                {
                    var item = entities.customers.Where(a => a.customerID == Login.id).SingleOrDefault();
        ...
        public void reload()
        {
            try
            {
                ...
                using (Entities entities = new Entities())
                {
                    var result = entities.customers.Where(n => n.customerID == Login.id).Select(n => n).ToList();
                    foreach (var x in result)
                    {
                        tbName.Text = x.name;
                        lbID.Text = "User ID: " + x.customerID;
        ...
        private void btnChEmail_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using(Entities entities = new Entities())
                {
                    var result = entities.customers.Where(a => a.emailAddress == tbEmail.Text).FirstOrDefault();
                    var result2 = entities.customers.Where(w => w.emailAddress == tbEmail.Text).FirstOrDefault();
                    ...
                    {
                        var state = entities.customers.SingleOrDefault(x => x.customerID == Login.id);
        ...
        private void btnChangePw_Click(object sender, EventArgs e)
        {
            using(Entities db = new Entities())
            {
                var state = db.customers.FirstOrDefault(x => x.customerID == Login.id);
