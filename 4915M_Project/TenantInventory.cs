...
{
    public partial class TenantInventory : UserControl
    {
        product product = new product();
        public TenantInventory()
        {
            InitializeComponent();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (Entities db = new Entities())
                {
                    ...
                    var condition = from a in db.tenants
                                    join b in db.showcases
                                    on a.tenantID equals b.tenantID
                                    join c in db.products
                                    on b.showcaseID equals c.showcaseID
                                    where b.showcaseID == product.showcaseID
                                    select b;

                    if (condition.SingleOrDefault() == null)
                    {
                        MessageBox.Show("The showcase is not belong to you");
                    }
                    else if (data == null)
                    {
        ...
        void populateDataGridView()
        {
            ...
            using (Entities db = new Entities())
            {
                var condition = from a in db.tenants
                                ...
                                select c;
                dgvProduct.DataSource = condition.ToList();
            }
        }
